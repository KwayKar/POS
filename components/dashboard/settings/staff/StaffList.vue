<template>
  <div ref="panelRefHeight" style="height: 100%">
    ...
    >
      <div
        v-for="(member, index) in staffStore.staffList"
        :key="member.name"
        class="staff-row"
        @click="onSelectStaff(member.id)"
        :style="index === staffStore.staffList.length - 1 ? 'padding-bottom: 5rem;' : ''"
      >
        <div class="staff-left">
          ...
          <div class="staff-info">
            ...
            <div class="staff-extra-mobile">
              <span class="staff-role-mobile">{{ member.role }}</span>
              <span class="staff-group-mobile">{{ member.group }}</span>
            </div>
          </div>
        </div>
